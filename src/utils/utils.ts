 ...
 * @LastEditTime: 2022-01-07 14:42:56
 * @Description: 工具类方法
 */
...
export const formatQ2S = function (params: pqoParamsProp) {
  const { pages, querys, orders } = params;
  // 分页参数兼容性处理
...
export const parseJSON = function (val: string, defaults = []) {
  let data;
  try {
    data = JSON.parse(val);
  } catch (error) {
